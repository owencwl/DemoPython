...
def main():
    ...
    with tf.variable_scope('layer1_conv1') as  scope:
        conv1_weight=tf.get_variable('weight',[5, 5, 3, 64],initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv1_biases=tf.get_variable('biases',64,initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('layer2_conv2') as  scope:
        conv2_weight = tf.get_variable('weight', [5, 5, 64, 64], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    print(pool2)
    reshaped=tf.reshape(pool2,[-1,8*8*64])
    print(reshaped)

    with tf.variable_scope('full_contact1') as  scope:
        fc1_weight=tf.get_variable('weight',[8*8*64,384],initializer=tf.truncated_normal_initializer(stddev=0.1))
        # 加入正则化

...
class CapsConv(object):
    ...
    def __call__(self, input, num_outputs, kernel_size=None, stride=None):
        ...
        if not self.with_routing:
            ...
            assert input.get_shape() == [128, 20, 20, 256]
            ...
            for i in range(self.num_units):
                ...
                with tf.variable_scope('ConvUnit_' + str(i)):
                    caps_i = tf.contrib.layers.conv2d(input,
                                                      ...
                                                      self.stride,
                                                      padding="VALID")
                    caps_i = tf.reshape(caps_i, shape=(128, -1, 1, 1))
                    capsules.append(caps_i)

            assert capsules[0].get_shape() == [128, 1152, 1, 1]

            # [batch_size, 1152, 8, 1]
            capsules = tf.concat(capsules, axis=2)
            capsules = squash(capsules)
            assert capsules.get_shape() == [128, 1152, 8, 1]

        else:
            # the DigitCaps layer
            # Reshape the input into shape [batch_size, 1152, 8, 1]
            self.input = tf.reshape(input, shape=(128, 1152, 8, 1))
            ...
            assert capsules.get_shape() == [128, 10, 16, 1]
...
def capsule(input, b_IJ, idx_j):
    ...
    with tf.variable_scope('routing'):
        ...
        for r_iter in range(3):
            ...
            assert s_j.get_shape() == [128, 1, 16, 1]
            ...
            assert u_produce_v.get_shape() == [128, 1152, 1, 1]
